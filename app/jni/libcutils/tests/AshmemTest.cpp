...
ASSERT_EQ(cfg
.ret, off) << "lseek(" << cfg.offset << ", " << cfg.whence << ") failed"
<< (errno ? ": " : "") << (errno ?
strerror(errno)
: "");

if (off >=
dataStart &&off<dataEnd
) {
...
ashmem_set_prot_region(fd, protFlags[i]
) >= 0) {
_exit(0);
} else {
_exit(1);
