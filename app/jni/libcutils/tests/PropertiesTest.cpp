...
#define PROPERTY_TEST_VALUE_DEFAULT "<<<default_value>>>"
    ...
    protected:
        ...
        int SetAndGetProperty(const char *value,
                              const char *defaultValue = PROPERTY_TEST_VALUE_DEFAULT) {
    ...
    // Null key -> unsuccessful set
{
    // Null key -> fails
    EXPECT_GT(0,
    property_set(/*key*/NULL,
    PROPERTY_TEST_VALUE_DEFAULT));
}
...
EXPECT_EQ(strlen("hello_world"), len
) << "hello_world key";
...
EXPECT_EQ(strlen(EMPTY_STRING_DEFAULT), len
) << "empty key";
...
EXPECT_EQ(PROPERTY_VALUE_MAX
-1, len) << "max length key";
EXPECT_STREQ(maxLengthString
.
...
EXPECT_GT(0, property_set(PROPERTY_TEST_KEY, oneLongerString.
...
EXPECT_EQ(strlen(VALID_TEST_VALUE), len
) << "set should've failed";
...
bool val = property_get_bool(PROPERTY_TEST_KEY, /*default_value*/true);
EXPECT_TRUE(val)
<< "Property should've been NEITHER (true) for string value: '" << valuesNeither[i] << "'";

val = property_get_bool(PROPERTY_TEST_KEY, /*default_value*/false);
EXPECT_FALSE(val)
<< "Property should've been NEITHER (false) for string value: '" << valuesNeither[i] << "'";
}
}
